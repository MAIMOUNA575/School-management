// Importation de toutes vos fonctions de gestion
import { addUser, listerUsers } from './services/UsersServices.js'; // Ajustez les chemins selon votre structure
...
function runTests() {
    ...
    addUser("Maimouna", "admin");
    addUser("Jean Prof", "professeur");
    addUser("Amadou", "etudiant");
    ...
    // On récupère les profs pour avoir leurs IDs générés par SQLite
    const profs = listerTeachers();
    const idProf1 = profs[0]?.id;
    const idProf2 = profs[1]?.id;

    // Ajout des matières liées aux profs
    addSubject("JavaScript ES6", idProf1);
    addSubject("SQL & SQLite", idProf2);
    ...
    addStudent("MAT-2026-01", "Koffi", "Amadou", 21, "Dev-Web");
    addStudent("MAT-2026-02", "Traoré", "Fatou", 22, "Dev-Web");
    ...
    // ----------------------------------------------------
    // /!\ Assurez-vous d'avoir corrigé le bug du `if (note >= 0 || note <= 20)` dans grades.js !
    ...
    if (idEtudiant1 && idMatiere1) {
        addAbsence(idEtudiant1, idMatiere1, "2026-06-15");
        addAbsence(idEtudiant1, idMatiere1, "2026-06-16");
